...
public class Trans3 extends Thread{
	...
	public Trans3(ReentrantLock bufferLock,HashSet<String> personSet,HashMap<String, Integer> bufferSet,String name1,String name2){
		this.name1 = name1;
		this.name2 = name2;
	...
	public void run(){
		...
		while(opr.exists(name1,name2,bufferSet)==true){
			synchronized (bufferSet){
				try {
					System.out.println("Trans3 Waiting <<element exists in bufferset>> ");
					bufferSet.wait();
				...
				System.out.println("Trans3 Waiting finish  <<check element again>> ");
