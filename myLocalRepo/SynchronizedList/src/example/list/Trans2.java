...
public class Trans2 extends Thread{
	...
	public Trans2(ReentrantLock bufferLock,HashSet<String> personSet,HashMap<String, Integer> bufferSet,String name1,String name2){
		this.name1 = name1;
		this.name2 = name2;
	...
	public void run(){
		...
		while(opr.exists(name1,name2,bufferSet)==true){
			synchronized (bufferSet){
				try {
					System.out.println("Trans2 Waiting <<element exists in bufferset>> ");
					bufferSet.wait();
				...
				System.out.println("Trans2 Waiting finish  <<check element again>> ");
